...
<TodoHeapApp>
  <MenuBarWidget>wchen@g.com | Sign Out | Last Sync: 2 minutes ago</MenuBarWidget>
  <ChatWidget>
    <ChatDisplayWidget>
      <MessageListWidget messages={}>
        <MessageWidget message={text: string, id: string, ref: number}>
        </MessageWidget>
        <MessageWidget message={text: string, id: string, ref: number}>
        </MessageWidget>
      </MessageListWidget>>
...
## actions
l, list -n [5], --reverse
a, add whatever text it is -p [0]
e, edit 3 (brings up the commit message)
d, delete 2 --force (actually removes it)
c, commit 23a4e message blah blah
...
todo: {
  ...
  text: string,
  timeCreated: PST string,
  status: "NotDone" | "Done" | "Deleted",
  timeDone: PST string,
  timeDeleted: PST string,
}

## module
- backing_store
 just an interface
 google_sheets_backing_datastore
- main
- query
- entity
  todo_entity
